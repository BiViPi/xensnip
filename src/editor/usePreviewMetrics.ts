...
export function usePreviewMetrics(
  ...
  canvasDocument: CanvasDocument | null = null
) {
  return useMemo(() => {
    const singleImageDims = image
    ...
    const centerX = ("drawX" in dims ? dims.drawX + dims.drawW / 2 : dims.canvasW / 2) * previewScale;
    const centerY = ("drawY" in dims ? dims.drawY + dims.drawH / 2 : dims.canvasH / 2) * previewScale;
    ...
    return result;
  }, [image, preset, viewportSize, panelReserveWidth, canvasDocument]);
}
